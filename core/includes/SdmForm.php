            ...
            switch ($value['type']) {
                ...
                case 'radio':
                    ...
                    foreach ($radioItems as $radio => $radioValue) {
                        $this->formElementHtml[$value['id']] .= '<' . (isset($value['displayOptions']['labelTag']) === true ? $value['displayOptions']['labelTag'] . ' id="' . $value['id'] . $radioIdIncrementer . '" ' : 'label for="SdmForm[' . $value['id'] . ']"') . '>' . $radio . '</' . (isset($value['displayOptions']['labelTag']) === true ? $value['displayOptions']['labelTag'] : 'label') . '><input type="radio" ' . (isset($value['displayOptions']['style']) === true ? 'style="' . $value['displayOptions']['style'] . '"' : '') . 'name="SdmForm[' . $value['id'] . ']" value="' . (substr($radioValue, 0, 8) === 'default_' ? $this->sdmFormEncode(str_replace('default_', '', $radioValue)) . '" checked="checked"' : $this->sdmFormEncode($radioValue) . '"') . '><br><!-- close form element "SdmForm[' . $value['id'] . ']" -->';
                        $radioIdIncrementer++;
                    }
