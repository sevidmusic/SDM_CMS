...
switch ($sdmcore->determineRequestedPage()) {
    ...
    case 'errors': // display recent errors
        $sdmassembler_contentObject->content->$sdmassembler_requestedpage->main_content .= trim('<h1>Site Errors</h1>' . str_replace('[', '<p style="overflow:auto;border: 2px solid black;border-radius: 3px;background: black;color: #CC0066; margin: 3px 3px 3px 3px; padding: 23px 23px 23px 23px;">', str_replace('<br />', '</p>', nl2br(file_get_contents($sdmcore->getCoreDirectoryPath() . '/logs/sdm_core_errors.log')))));
        break;
