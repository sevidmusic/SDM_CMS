...
foreach ($categorizedExpenses as $category => $categoryExpenses) {
    $categorizedExpensesTable .= '<tr><th style="text-align: left; color: #000;">' . $category . '</th></tr>';
    foreach ($categoryExpenses as $expense => $amount) {
        $bgColor = ($color === true ? '#004C9F' : '#007999');
        $categorizedExpensesTable .= '<tr style="background: ' . $bgColor . ';"><td id="rounded-left">' . $expense . '</td><td class="budget-text-center" id="rounded-right">$' . $amount . '</td></tr>';
