...
try {
    ...
    */
    ...
    if (move_uploaded_file($_FILES['SdmForm']['tmp_name']['sdmMediaFile'], $savePath . '/' . $uniqueFileName) !== false) {
        /* upload succeed. */
        $fileUploadStatus = 'Sdm Media Displays File Upload Status: File was uploaded successfully.' . PHP_EOL;
        /* Report to upload log */
        $sdmMediaUploadLog .= $sdmassembler->sdmCoreSdmReadArrayBuffered(['$fileUploadStatus' => $fileUploadStatus]);
    } else {
...
$sdmMediaUploadLogMessage = $errorMessages . PHP_EOL . $fileUploadStatus . PHP_EOL . $sdmMediaUploadLog;
/* Catch and log any error messages and assign to $fileUploadStatus. */
