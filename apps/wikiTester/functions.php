 ...
 * @return string
 */
function wikiArrayToList($array, $parentKey, $rowcolor = '#000000') {
    ...
    foreach ($array as $key => $value) {
        switch (is_array($value)) {
            case TRUE:
                $list .= wikiArrayToList($value, $key, $rowcolor);
                break;
            case FALSE:
...
function assemlbeOmdbTableElements($embedlyData, $rowcolor, $testurl = 'unknown') {
    $sdmcore = new SdmCore();
    $styles_wikiDataTableRow = 'background: ' . $rowcolor . '; color: ' . ($rowcolor === '#363636' ? '#D0D0D0' : '#F0F0F0') . ';'; // @todo alternate bg color every row
    ...
    $wikiApiRequestUrls = array(
        'http://www.wikiapi.com/?t=' . str_replace(array(' ', '_', '+'), '-', urlencode((isset($decodedEmbedlyData['title']) ? $decodedEmbedlyData['title'] : 'null'))),
        'http://www.wikiapi.com/?t=' . str_replace(array(' ', '_', '+'), '-', urlencode((isset($decodedEmbedlyData['title']) ? $decodedEmbedlyData['title'] : 'null'))) . '&type=movie',
        'http://www.wikiapi.com/?t=' . str_replace(array(' ', '_', '+'), '-', urlencode((isset($decodedEmbedlyData['title']) ? $decodedEmbedlyData['title'] : 'null'))) . '&type=episode',
        'http://www.wikiapi.com/?t=' . str_replace(array(' ', '_', '+'), '-', urlencode((isset($decodedEmbedlyData['title']) ? $decodedEmbedlyData['title'] : 'null'))) . '&type=series',
    );
    // Create Header Rows
    $output = '<tr style="' . $styles_wikiDataTableRow . '"><td style="' . $styles_td . '">Provider Name : ' . (isset($decodedEmbedlyData['provider_name']) ? $decodedEmbedlyData['provider_name'] : '<b style="color:red;">UNKNOWN PROVIDER OR BAD URL | Tested URL : <i><a href="' . $testurl . '">' . $testurl . '</a></i></b>') . '</td></tr>';
    $output .= '<tr style="' . $styles_wikiDataTableRow . '"><td style="' . $styles_td . '">Movie Url : <i><a href="' . $testurl . '">' . $testurl . '</a></i></td></tr>';
    $output .= '<tr style="' . $styles_wikiDataTableRow . '"><td style="' . $styles_td . '">Movie Title : <i>' . (isset($decodedEmbedlyData['title']) ? $decodedEmbedlyData['title'] : '<b style="color:red;">Title Unknown</b>') . '</i></td></tr>';
    // Create Returned Data Rows
    foreach ($wikiApiRequestUrls as $index => $wikiApiRequestUrl) {
        $output .= '<tr style="' . $styles_wikiDataTableRow . '"><td style="' . $styles_td . '">OMDB API Request Url <i style="color:#95B9C7;">(' . ($index === 0 ? 'no type' : ($index === 1 ? 'type=movie' : ($index === 2 ? 'type=episode' : ($index === 3 ? 'type=series' : '')))) . ')</i> : <i>' . $wikiApiRequestUrl . '</i></td></tr>';
        /** OMDB TESTS * */
        // meke request
        $wikiJsonString = $sdmcore->sdmCoreCurlGrabContent($wikiApiRequestUrl);
        // add a row to show the json string that was returned
        $output .= '<tr style="' . $styles_wikiDataTableRow . '"><td style="' . $styles_td . '">Returned JSON String : <xmp style="color:orange;">' . $wikiJsonString . '</xmp></td></tr>';
        // decode the json string
        $decodedOmdbData = json_decode($wikiJsonString, TRUE);
        // BUILD DATA HEADER ROW //
        $output .= '<tr style="' . $styles_wikiDataTableRow . '">';
            ...
            switch (is_array($value)) {
                case TRUE:
                    if (empty($value)) {
                        ...
                        $output .= '<td style="' . $styles_td . '"><div style="height:200px;min-width:420px;overflow:auto;">' . (is_null($value) || $value === '' || $value === 'N/A' || $value === 'Null' || $value === 'null' || $value === 'False' || $value === 'Movie not found!' ? '<i style="color:aqua">null</i>' : wikiArrayToList($value, $key, $rowcolor)) . '</div></td>';
                    }
                    break;
                default:
...
function buildOmdbDataTable($movieUrls) {
    ...
    $sdmcore = new SdmCore();
    // only show wiki data if form was submitted or we are in test mode, i.e., if $movieUrls is empty do NOT display wiki data table
    if (!empty($movieUrls)) {
        $styles_wikiDataTable = 'background:black;border:2px solid #767676; border-radius: 9px;';
        $embedlyDataTable .= '<table id="wikiDataTable" style="' . $styles_wikiDataTable . '">';
        $rowcolor = '#363636'; // initial row color | will alternate
        // create rows and columns of wiki data
        foreach ($movieUrls as $movieUrl) {
            $embedlyRequestUrl = $embedlyUrl . $movieUrl;
            $wikiData = $sdmcore->sdmCoreCurlGrabContent($embedlyRequestUrl, array());
            $embedlyDataTable .= assemlbeOmdbTableElements($wikiData, $rowcolor, $movieUrl);
...
function getOmdbTestForm() {
    $sdmcore = new SdmCore();
    $wikiTesterForm = new SdmForm();
    $wikiTesterForm->form_handler = 'wikiTester';
    $wikiTesterForm->method = 'post';
    $wikiTesterForm->submitLabel = 'See What Embedly Returns For This Movie';
    $wikiTesterForm->form_elements = array(
        array(
            'id' => 'movieUrl',
            'type' => 'text',
            'element' => 'Movie Url',
    ...
    $wikiTesterForm->sdmFormBuildForm($sdmcore->sdmCoreGetRootDirectoryUrl());
    return $wikiTesterForm->sdmFormGetForm();
}
